<?php 
 ...
 if (strpos($_SERVER["PHP_SELF"] , $seccion_trabajo) >1 ) {
  ...
  include ($absolute_include."modelos/historiales_personal/model.historiales_personal.php");
  ...
  function archivos_index(){
    ...
    $resultado_carreras = buscar_historiales_personal();
       
    // llama a la vista de index de archivos

    include ($absolute_include."vistas/consultas/historiales_personal/historiales_personal.php"); 
  ...
  function ir_historial($arg_POST){
    ...
    $resultado_carreras = buscar_historiales_personal2($arg_POST['personal_id']);
    //var_dump($resultado_carreras);
    include ($absolute_include."vistas/consultas/historiales_personal/historiales_personal2.php"); 
  }
  ...
  function editar_historial($arg_POST){
    ...
    $resultadoBusquedaCarrera = buscar_historiales_personal($arg_POST['historialpersonal_id']);
    //var_dump($resultadoBusquedaCarrera);
    include ($absolute_include."vistas/consultas/historiales_personal/editar.php"); 
  }
  ...
  function actualizar_carrera($arg_POST){
    ...
    $actualizarCarrera = modificar_datos($arg_POST['historialpersonal_id'], $arg_POST['historial_personal']);
    
    if ($actualizarCarrera) {
      echo '<script language = javascript>alert("Se ha modificado el historial correctamente")
        self.location =  "'.$carpeta_trabajo.'/controladores/consultas/historiales_personal/controller.historiales_personal.php ?>"
      </script>';
    }else{
